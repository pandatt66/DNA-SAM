...
x_norest_$(1).tpr:x_temp.mdp
	gmx grompp -f x_temp.mdp -c md_x1_$(1).gro -r ref_md_boxz_$(1).pdb -p cooc_md_$(1).top -n index_cooc_$(1).ndx -o x_norest_$(1).tpr -maxwarn 1 
...
$(1)_1_coord_fixed.dat:$(1)_1_coord.xvg md_x1_$(1).gro 
...
$(foreach n,$(NUCLEOTIDES),$(eval $(call ASSOA_NUM,$(n))))\
$(foreach n,$(NUCLEOTIDES),$(eval $(call ASSOA_FIXED,$(n))))\
#################################################
#dNMP-COOC minimum distance
#################################################
...
$(1)_1_ads_times.dat: read_xvg.py ads_des_times.py ads_des_stats.py $(1)_1_coord_fixed.dat $(1)_1_numcont.xvg
	python ads_des_stats.py $(1)_1_mindist.xvg $(1)_1_coord_fixed.dat $(1)_1 -ic $(1)_1_numcont.xvg -mt 0.3

endef

$(foreach n,$(NUCLEOTIDES),$(eval $(call ADS_DES_STATS,$(n))))

##octave: sep_dist_timeblcks(equiltime)###

define ASSOA_NUM

$(1)_1_assoa_numcont.xvg:
	echo 12 5 | gmx mindist -f md_x1_$(1).xtc -s x_nonwater_$(1).tpr -n x_nonwater_$(1).ndx -od $(1)_1_assoa_minidist.xvg -on $(1)_1_assoa_numcont.xvg -d 0.4
...
$(1)_1_assoa_numcont_fixed.dat:
	python fix_assoa_num.py $(1)_1_assoa_numcont.xvg $(1)_1
...
#####################
define NDX_BASE
index_cooc_$(1).ndx:make_ndx.bash
	source make_ndx.bash md1_cooc_new_$(1).gro $(1) cooc
	source monolayer_selections_to_index_file.bash $(1) COOC SH 35 cooc
	source monolayer_selections_to_index_file.bash $(1) COOC C13 35 cooc
endef

$(foreach n,$(NUCLEOTIDES),$(eval $(call NDX_BASE,$(n))))

define CTR_SH
$(1)_ctr_SH.trr:index_cooc_$(1).ndx
	echo 9 0|gmx trjconv -f md_x1_$(1).trr -s md_x1_$(1).tpr -o $(1)_ctr_SH.trr -pbc mol -n index_cooc_$(1).ndx -center
...
pmf_$(1)_SH.xvg:$(1)_ctr_SH.trr
	echo 11|gmx traj -f $(1)_ctr_SH.trr -s md_x1_$(1).tpr -n index_cooc_$(1).ndx -com -ox -nox -noy pmf_$(1)_SH.xvg
endef
...
define PMF_BASE
pmf_$(1)_base.xvg:$(1)_ctr_SH.trr pmf_$(1)_SH.xvg $(1)_ctr_SH.trr
	echo 10|gmx traj -f $(1)_ctr_SH.trr -s md_x1_$(1).tpr -n index_cooc_$(1).ndx -com -ox -nox -noy pmf_$(1)_base.xvg
endef
...
# for dCMP cross the half box, to check pmf with non-equil, wham opition -min 1.56 -max 2.76
############################################################
define EXT_CONFIG
ref_pmf_$(1).pdb:trajconv_dump.bash pmf_$(1)_time.dat
	source trajconv_dump.bash $(1) pmf_$(1)_time.dat -3.552 cooc
endef
...
###########################################################
##Compare prob from PMF with prob from non-equil simulation for all bins 
#put the solvent in the center of box. 
##############################

define CTR_SOL
$(1)_ctr_SOL.trr:$(1)_ctr_SH.trr
	echo 0|trjconv -f $(1)_ctr_SH.trr -s md_x1_$(1).tpr -pbc mol -n index_cooc_$(1).ndx -trans 0 0 -3.552 -o $(1)_ctr_SOL.trr
...
pmf_$(1)_SH1.xvg:$(1)_ctr_SOL.trr
	echo 11|gmx traj -f $(1)_ctr_SOL.trr -s md_x1_$(1).tpr -n index_cooc_$(1).ndx -com -ox -nox -noy pmf_$(1)_SH1.xvg
endef
...
pmf_$(1)_SH2.xvg:$(1)_ctr_SOL.trr pmf_$(1)_SH1.xvg
	echo 12|gmx traj -f $(1)_ctr_SOL.trr -s md_x1_$(1).tpr -n index_cooc_$(1).ndx -com -ox -nox -noy pmf_$(1)_SH2.xvg
endef
...
pmf_$(1)_base_ctr.xvg:$(1)_ctr_SOL.trr pmf_$(1)_SH2.xvg
	echo 10|gmx traj -f $(1)_ctr_SOL.trr -s md_x1_$(1).tpr -n index_cooc_$(1).ndx -com -ox -nox -noy pmf_$(1)_base_ctr.xvg
endef
